...
- The step count would exceed 500 (internal index > 998).
...
Stops the benchmark, assembles a formatted report string, logs it via `sh.log` under the `'benchmark'` label, and deletes the benchmark from memory.
...
<Warning>
  Each benchmark supports a maximum of **500 steps**. Attempting to record a step when the internal index exceeds 998 (i.e. a 501st step) will trigger `sh.error()` and the step will not be recorded. If you need to profile loops longer than 500 iterations, consider sampling — for example, only calling `step()` every N iterations.
</Warning>
