---
title: "DOM Querying and CSS Helpers in DiWu"
...
Wraps `getElementById`. Returns the element whose `id` attribute matches `selector`, or `null` if none exists. Because `getElementById` is scoped to `document`, this is effectively a document-level lookup regardless of which element it is called on.
