---
title: "Async Utilities and Task Scheduling in TianFeng"
sidebarTitle: "Async"
...
Inside the callback, `this` (the `control` object) exposes a `stop()` method so the task can cancel itself without needing an external reference to its id:
